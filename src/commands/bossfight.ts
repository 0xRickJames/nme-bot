...
const loopData = new SlashCommandBuilder()
  .setName("bossfight")
  .setDescription("Fight against a Boss!")
  .addStringOption((option) =>
    option
      .setName("boss")
      .setDescription("Boss you'd like to fight")
      .setRequired(true)
      .addChoices(
...
export const bossfight: Command = {
  ...
  run: async (interaction: ChatInputCommandInteraction) => {
    ...
    const boss_option = interaction.options.get("boss");
    const boss = await powerUpCheckBoss(boss_option!.value as string);
